...
import java_core_hw_10.family.Man;
import java_core_hw_10.family.Woman;
...
public class CollectionFamilyDao implements FamilyDao {
    ...
    public void loadData(List<Family> familyList) {
        try (FileOutputStream fileOutputStream=new FileOutputStream("families.txt")){
           ...
           for(Family family:familyList){
               allfamilies+=family.prettyFormat();
           }
           byte[] bytes=allfamilies.getBytes();
    ...
    public String readFromTXT() {
        ...
        if(!file.exists()){
            return "families.txt not found";
        }
        try (FileInputStream fileInputStream=new FileInputStream("families.txt")){
            int charVal;
            String allfamilies="";
            while((charVal=fileInputStream.read())!=-1){
                char charToString=(char)charVal;
                allfamilies+=charToString;
            }
            return allfamilies;
        }catch (Exception e){
            e.printStackTrace();
